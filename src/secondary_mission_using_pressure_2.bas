...
z var word          ' Hulpvariabele voor pulsen
...
do
	...
	if a > mindruk then
		...
		flag = flag + 1
	    ' zorg ervoor dat de flag niet groter wordt dan 10
		if flag > 10 then
			flag = 10
		endif
	...
	' als er 3 keer de juiste meting wordt gedaan zijn we voorbij het omslagpunt
	if flag > 3 then
		...
		if bOntKoppel = 1 then
			...
			'start de do-while loop
			low P14
			pause 5000
			SERVO P2, 1200, 13
		'	do
			    'stuur een stroompje naar de servo
		'		high P2
				'korte pauze
		'		pause 100
				'even geen stroom sturen
		'		low P2
				'korte pauze
		'		pause 10
				'verhoog variabele z met 1
		'		z = z + 1
				'herhaal de loop 13 keer
		'	while z < 13
		endif
	endif
